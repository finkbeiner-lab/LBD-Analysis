    ...
    PathClass setObjectClass(PathObject object) {
       ...
       if (this.assignclass=="DELETE-Annotation") {
           ...
           if (object!=null && object.isLocked()==false){
               ...
               def annotations = imageData.getHierarchy().getAnnotationObjects().findAll { it.getROI().getRoiName()!="Rectangle" && it.isLocked()==false && it.getPathClass()!=null}
               
               for (int i = 0; i < annotations.size(); i++) {
                 if (i<9){
                      annotations[i].setName("0"+(i+1).toString()+":"+annotations[i].getPathClass())
                     }
                 else {
                     annotations[i].setName((i+1).toString()+":"+annotations[i].getPathClass())
                 }
                }
       ...
       def annotations = imageData.getHierarchy().getAnnotationObjects().findAll { it.getROI().getRoiName()!="Rectangle" && it.isLocked()==false && it.getPathClass()!=null}
        ...
        for (int i = 0; i < annotations.size(); i++) {
            if(annotations[i].getName()==null) {
                if (i<10){
                    annotations[i].setName("0"+(annotations.size()).toString()+":"+annotations[i].getPathClass())
                  }
                else {
                    annotations[i].setName((annotations.size()).toString()+":"+annotations[i].getPathClass())
                }
            }       
        }
