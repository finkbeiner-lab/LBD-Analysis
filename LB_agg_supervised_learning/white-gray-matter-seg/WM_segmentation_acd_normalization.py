...
import pyvips
import pickle
from Normalization.acd import ACDModel, acd_model
from Normalization.stain_normalizer import StainNormalizer
...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, img_list):
        ...
        transforms.ToTensor(),
        transforms.Normalize([0.8494, 0.8229, 0.8481],[0.1359, 0.1535, 0.1285])
...
def run_segmentation(img_path, tilesize,stride, BATCH_SIZE, model_lbd, model_wgm,device,normalizer,LBD_model_path,save_results_path, wgm_ground_truth=False):
    start = timer()
    pyvips_fn = pyvips_image(img_path, normalizer = None)
    ...
    normalized_crops = normalizer.transform(crops)
    
    masked_image = generate_mask_from_wgm_prediction(normalized_crops, xy_coords, model_wgm, device, orig_w, orig_h, tilesize, BATCH_SIZE)
...
if __name__ == '__main__':
    ...
    WM_model_path = '/gladstone/finkbeiner/steve/work/data/npsad_data/monika/LBD/WM_models/2zl6wmdu15.pth'
    ...
    save_results_path="/gladstone/finkbeiner/steve/work/data/npsad_data/monika/LBD/WM_segmentation/Evaluation_Metrics/"
    ...
    for i, img in enumerate(imgs):
        ...
        try:
            run_segmentation(img, tilesize,stride, BATCH_SIZE, model_lbd, model_wgm,device,normalizer,LBD_model_path, save_results_path, True)
