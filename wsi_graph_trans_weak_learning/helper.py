...
def preparefeatureLabel(batch_graph, batch_label, batch_adjs, n_features: int = 512):
    batch_size = len(batch_graph)
    #print("len(batch_graph)", batch_size)
    labels = torch.LongTensor(batch_size)
    max_node_num = 0
    
    for i in range(batch_size):
        labels[i] = batch_label[i]
        max_node_num = max(max_node_num, batch_graph[i][0].shape[0])
        #print(batch_graph[i][0].shape[0])
        #print(labels[i])
    ...
    for i in range(batch_size):
        cur_node_num =  batch_graph[i][0].shape[0]
        #node attribute feature
        tmp_node_fea = batch_graph[i][0]
        batch_node_feat[i, 0:cur_node_num] = tmp_node_fea
...
class Trainer(object):
    ...
    def train(self, sample,i_batch, model, n_features: int = 512):
        node_feat, labels, adjs, masks = preparefeatureLabel(sample['image'], sample['label'], sample['adj_s'], n_features=n_features)
        pred,labels,loss = model.forward(node_feat, labels, adjs, masks, i_batch)
...
class Evaluator(object):
    ...
    def eval_test(self, sample,i_batch, model, graphcam_flag=False, n_features : int = 512):
        node_feat, labels, adjs, masks = preparefeatureLabel(sample['image'], sample['label'], sample['adj_s'], n_features=n_features)
        if not graphcam_flag:
            ...
            pred,labels,loss= model.forward(node_feat, labels, adjs, masks,i_batch, graphcam_flag=graphcam_flag)
        return pred,labels,loss