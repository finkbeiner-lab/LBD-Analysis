...
class Classifier(nn.Module):
    ...
    def forward(self,node_feat,labels,adj,mask,i_batch, is_print=False, graphcam_flag=False):
        ...
        embeds=0
        concats=[]
        filepath = 'graphcam/kfold_output_train0/{}'.format(i_batch)
        ...
        if graphcam_flag:
            ...
            torch.save(s_matrix, path.join(filepath, 's_matrix.pt'))
            torch.save(s[0], path.join(filepath, 's_matrix_ori.pt'))
        ...
        if graphcam_flag:
            ...
            p = F.softmax(out)
            torch.save(p, path.join(filepath, 'prob.pt'))
            ...
            for index_ in range(p.size(1)):
                ...
                torch.save(cam, path.join(filepath, 'cam_{}.pt'.format(index_)))
